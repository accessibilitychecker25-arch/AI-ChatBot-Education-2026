...
export class BatchUploadComponent implements OnDestroy {
  ...
  onFilesSelected(ev: Event) {
    ...
    // 2. Enforce PowerPoint File Type
    // Regex matches .pptx, .ppt, .pps, or .potx (case insensitive)
    const validExtensions = /\.(pptx|ppt|pps|potx)$/i;
    ...
    // If all checks pass, accept the files
  ...
  upload() {
    ...
    // Double check limit before sending (safety net)
    if (this.files.length > 7) {
      this.error = 'Too many files selected (Max 7)';
      return;
    }
